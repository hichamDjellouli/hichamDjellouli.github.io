(function () {
    ...
    CalendrierController.$inject = ['$rootScope', 'PatientsService', '$route', '$templateCache', 'filterFilter', '$timeout', '$compile', 'moment', 'calendarConfig'];
    function CalendrierController($rootScope, PatientsService, $route, $templateCache, filterFilter, $timeout, $compile, moment, calendarConfig) {
        var vm = this;
                    ...
                    if (resultConfirmation.value) {
                        PatientsService.Add_Patient(New_Patient)
                            .then(function (result) {
                                ...
                                //console.log('CalendrierController -> users error : ' + result);
                                toastr.error('Erreur apparue lors de l\'insertion');

                            })//.catch(angular.noop);
                            .catch(function (error) {
                                // handle errors
                                //console.log('CalendrierController -> Un probleme est survenu : ' + error);
                                toastr.error('Erreur apparue lors de l\'insertion');
                            });
                    } else {
                        swal.close();
